...
def _traffic_kind(metric_type: str) -> str | None:
    """Return 'interface' or 'nlbw' when metric is a traffic counter topic."""
    for kind in ("interface", "nlbw"):
        if metric_type.startswith(f"{kind}-") and metric_type.endswith(_TRAFFIC_SUFFIXES):
            return kind
    return None


def _traffic_label(metric_type: str) -> str:
    """Human label from topic segment (interface name or nlbw device slug)."""
    head = metric_type.split("/")[0]
    label = head.split("-", 1)[1] if "-" in head else head
    if re.fullmatch(r"[0-9a-f]{12}", label, re.IGNORECASE):
        return ":".join(label[i : i + 2] for i in range(0, 12, 2))
    ...
    async def async_added_to_hass(self):
        ...
        def message_received(message):
            ...
            if parsed_value is not None:
                ...
                if kind:
                    parts = metric_type.split('/')
                    attr_key = "device" if kind == "nlbw" else "interface"
                    self._extra_attributes = {
                        attr_key: _traffic_label(metric_type),
                        "metric": parts[1] if len(parts) > 1 else "",
                    }
...
class OpenWrtMQTTRateSensor(SensorEntity):
    ...
    def _parse_payload(self, payload):
        ...
        try:
            # Network / nlbw traffic: extract RX or TX based on direction
            if _traffic_kind(metric_type):
                match = re.search(r'rx:([\d]+),tx:([\d]+)', payload)
                if match:
